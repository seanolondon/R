...
addBaseMerge2 <- mutate(addBaseMerge2, update_date = today)
...
print("start test")
conn <- dbConnect("PostgreSQL",dbname='gla_gis',host='gispostsqlaws.cbebbtl0e2o3.eu-west-1.rds.amazonaws.com',port='5432',user='gisapdata',password='gi$own')
postTables <- dbListTables(conn)
countTable <- postTables[stringr::str_detect(postTables, pattern = "addbase_ldn")]
gdbLayers <- ogrListLayers("abp.gdb")
geodatTestFinal <- stringr::str_detect(gdbLayers, pattern = paste0("addressBasePlusLondon_",today))

sink(file = "D:\\FME Scheduled_tasks\\R_tasks\\abaseplus_automation_v0.9_log.txt", split = TRUE, append = FALSE)

if(geodatTestFinal > 0 && countTable == "addbase_ldn"){
...
print("NO UPDATE TO ADDRESS BASE PLUS")P
print("SUCCESSFUL")
