...
addBaseMerge3BNGLDN %>%
arc.write(path = paste0("abp.gdb/addressBasePlusLondon_",today))

addBaseMerge3BNG %>% 
arc.write(path = paste0("abp.gdb/addressBasePlus_",today))
...
geodatTest <- nrow(readOGR(dsn = "abp.gdb", layer = paste0("addressBasePlusLondon_",today)))

sink(file = "D:\\FME Scheduled_tasks\\R_tasks\\abaseplus_automation_v0.9_log.txt", split = TRUE, append = FALSE)

if(geodatTest > 1000 && countTable == "addbase_ldn"){
