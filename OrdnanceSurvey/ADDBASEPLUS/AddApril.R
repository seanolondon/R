...
session2 <- jump_to(session1, url = "https://www.ordnancesurvey.co.uk/orderdownload/orders?id=&idH=&idE=&size=10&filter=0&sort=8&page=1")

#follow first download link to first page
linkText1 <- session2 %>% 
  follow_link(i = 26) %>% ## this returns the first order, 20 = 3rd order, 19 = 2nd order, 18 = 1st order
...
#follow first download link to first page
download1 <- session2 %>% 
  follow_link(i = 26) %>% ## 20 = 3rd order, 19 = 2nd order, 18 = 1st order
